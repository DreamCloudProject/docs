---
title: 'Introduction'
description: 'How to call MastermindCMS APIs (WebSocket/STOMP and REST)'
---

This section explains the two API styles used by MastermindCMS clients:

- WebSocket API (STOMP) for interactive, event-driven requests and SSR/UI updates
- REST API for authentication and file/data operations (and an HTTP fallback for service calls)
...
- WebSocket JSON handlers can accept a JWT via the native header `Authorization: Bearer <token>`.
