...
| Аргумент | Тип | Описание |
| --- | --- | --- |
| `emailData` | `LinkedHashMap<String,Object>` | Передаётся через `args[0].0`. |
...
<ResponseField name="addAttachment" type="boolean">
...
| `attachment` | `Attachment` | Передаётся через `args[2].2`. |
...
<ResponseField name="prepareItemsToShipment" type="boolean">
...
```json
{
  "beanId": "orderServiceImpl",
  "scope": "PROTOTYPE",
  "functionName": "prepareItemsToShipment",
  "args": [
    {
      "0": null
    }
