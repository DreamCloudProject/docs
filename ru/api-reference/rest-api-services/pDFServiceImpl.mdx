...
description: 'Вызов публичных методов сервиса через REST API: pDFServiceImpl'
---
...
| Аргумент | Тип | Описание |
| --- | --- | --- |
| `headerHtml` | `String` | Передаётся через `args[0].0`. |
| `bodyHtml` | `String` | Передаётся через `args[1].1`. |
| `footerHtml` | `String` | Передаётся через `args[2].2`. |
